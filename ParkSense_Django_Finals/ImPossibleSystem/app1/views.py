...
def HomePage(request):
    ...
    car_stats = {
        'total': car_slots.filter(status='Available').count(),  # Use correct capitalization
        'available': car_slots.filter(status='Available').count(),
        'occupied': car_slots.filter(status='Occupied').count(),
        'maintenance': car_slots.filter(status='Under Maintenance').count(),
        'slots': car_slots
    }
    
    motorcycle_stats = {
        'total': motorcycle_slots.filter(status='Available').count(),  # Use correct capitalization
        'available': motorcycle_slots.filter(status='Available').count(),
        'occupied': motorcycle_slots.filter(status='Occupied').count(),
        'maintenance': motorcycle_slots.filter(status='Under Maintenance').count(),
